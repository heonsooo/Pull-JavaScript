...
// Number
let num: Number;
let numberoZero: number = 0;
let floatNumber: number = 0.2;

// String
let string: string;
let stringName: string = "Soo";
let firstName: string = "Doo";
...
let mixedArray2: Array<string | number> = ["a", 1];

// Array : any (타입을 지정할 수 없을 경우)
let anyArray: any[] = [1, "a"];
let anyArray2: Array<any> = [1, "a"];

// Array : 읽기 전용 배열 (readonly , ReadonlyArray)
let ReadonlyStringArray: readonly string[] = ["a", "b", "c"];
let ReadonlyNumberArray: ReadonlyArray<number> = [1, 2, 3];
...
let tupleOfArray: [number, string][] = [
  [1, "a"],
  [2, "b"],
];
...
isNumber = isUnknown as number; // Type Assertion(타입 단언 : 개발자가 직접 해당 변수의 타입 지정하여 에러를 발생시키지 않도록 하는 것)을 통해 할당 가능하다.

let abcd: string = "abcd";
isNumber = abcd as unknown as number; // ?????????
...
let func2 = function (x: number, y: number): number {
  return x + y;
};
...
let func3: (a: string, b: string) => void;
func3 = function (a, b): void {
  // console.log(a, b);
};
// func3("aab", "asd");
...
let voidValue: void = undefined; // 하지만, void 타입에는 undefined 타입만은 할당 가능하다.
...
function returnVoid3(msg: string): undefined {
  console.log(msg);
  // return undefined;
}
